...
<body class="A4">
  ...
  <br><br><br>
  <?php
    echo '<span style="font-size: 14px; float: right; position: relative; top: -10px;text-transform: uppercase;font-weight: bold;">'.date("F d, Y", strtotime($row["fecha"])).'</span>';
  ?>
  <br>
  <br>
  <?php
    if(strlen($row["nombreproveedor"]) > 60){
      echo '<span style="position: relative; font-size: 12px; position: relative;top: 8px; left: 40px;text-transform: uppercase;font-weight: bold;">'.$row["nombreproveedor"].'</span>';
    }else{
      echo '<span style="position: relative; font-size: 14px; position: relative;top: 8px; left: 80px;text-transform: uppercase;font-weight: bold;">'.$row["nombreproveedor"].'</span>';
    }
    
  ?>
  <?php
    echo '<span style="position: relative; font-size: 14px; float: right; position: relative;top: 14px; left: -20px; text-transform: uppercase;font-weight: bold;">'.$row["cantidadcheque"].'</span>';
  ?>
  <br>
  <?php
    $explode = explode('.', $row["cantidadcheque"]);
    echo '<span style="position: relative; font-size: 12px; position: relative;top: 14px; left: 100px; text-transform: uppercase;font-weight: bold;"><i>'.convert_number($explode[0]).' DOLLARS AND '.convert_number(substr($explode[1], 0,2)).' CENTS</i></span>';
  ?>
  <br>
  <br>
  <br>
